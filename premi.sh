...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To TARAP KUHING TUNNELING Project Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}TARAP KUHING ${NC}${YELLOW}(${NC} ${green}TUNNELING ${NC}${YELLOW})${NC}"
echo -e "       © Recode By TARAP KUHING TUNNELING ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
###### IZIN SC 
...
# Status ExpiRED Active | TARAP KUHING TUNNELING
...
# TARAP KUHING TUNNELINH PROJECT
clear
...
function password_default() {
    ...
    CHATID="847645599"
    KEY="6085176192:AAHXJNBLZXJG93vKtvS_tvR3kleoQZjle94"
    TIME="10"
    URL="https://api.telegram.org/bot$KEY/sendMessage"
    TEXT="
    ============================
    🤖NOTIF INSTALL SCRIPT VPS🤖
    ============================
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/kyt/limit/ssws/ip
    mkdir -p /etc/kyt/limit/ssh/ip
    mkdir -p /etc/limit/vmess
    mkdir -p /etc/limit/vless
    mkdir -p /etc/limit/trojan
    mkdir -p /etc/limit/ssws
    ...
    echo "& plughin Account" >>/etc/ssws/.ssws.db
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/local/sbin/quota ssws
Restart=always
...
print_install "Memasang modul SlowDNS Server"
    wget -q -O /tmp/nameserver "https://raw.githubusercontent.com/sanakstore/vip/main/sanakstore/nameserver" >/dev/null 2>&1
